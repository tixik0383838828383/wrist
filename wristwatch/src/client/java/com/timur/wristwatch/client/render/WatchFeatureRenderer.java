...
/**
 * Рисует небольшой индикатор с содержимым хотбара рядом с головой игрока — то, что видят
 * ДРУГИЕ игроки, когда кто-то поднял руку с часами (у себя игрок видит полноценную плавающую
 * HUD-панель — см. WatchHudOverlay). Прогресс анимации и активность берутся из ClientWatchState,
 * куда попадают данные с сервера по сети.
 *
 * Крепится именно к ГОЛОВЕ, а не к руке: голова почти у всех моделей (в т.ч. кастомных Figura)
 * остаётся близко к ванильной позиции скелета, а кисть/предплечье могут быть переопределены
 * кастомной моделью совершенно произвольно. Если у Figura-аватара (например, Stick Figures)
 * рендер полностью заменён Lua-скриптами, сама Figura-модель может не реагировать на нашу
 * анимацию руки вообще — это ограничение Figura, а не баг мода; индикатор у головы при этом
 * всё равно будет на своём месте, так как берётся из ванильного скелета.
 */
